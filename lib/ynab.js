    ...
    try {
      await ynabAPI.transactions.updateTransactions(this.budget.id, {
        transactions: matches.map((m) => {
          ...
          if (m.order.items.length === 1) {
            // Single item: truncate if too long
            memo = m.order.items[0];
            if (memo.length > maxLength) {
              memo = memo.substring(0, maxLength - 3) + "...";
            }
          } else {
            // Multiple items: show count and first item
            const firstItem = m.order.items[0];
            const itemCount = m.order.items.length;
            memo = `${itemCount} items: ${firstItem}`;

            if (memo.length > maxLength) {
              // Truncate the first item name to fit
              const prefix = `${itemCount} items: `;
              const availableLength = maxLength - prefix.length - 3; // -3 for "..."
              memo = prefix + firstItem.substring(0, availableLength) + "...";
            }
          }
