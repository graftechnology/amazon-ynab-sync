    ...
    if (items.length === 0) {
      ...
      $("table tr").each((i, row) => {
        ...
        if (
          ...
          !rowText.toLowerCase().includes("hello") &&
          !rowText.toLowerCase().includes("thank you") &&
          !rowText.toLowerCase().includes("arriving") &&
          !rowText.toLowerCase().includes("ship to") &&
          !rowText.toLowerCase().includes("email was sent") &&
          !rowText.toLowerCase().includes("privacy notice") &&
          !rowText.toLowerCase().includes("confirmation") &&
          (rowText.includes("Qty") || rowText.includes("..."))
        ) {
          // Extract the longest text content from the row that looks like a product
          let longestText = "";
          $(row)
            .find("td, th")
            .each((j, cell) => {
              ...
              if (
                ...
                cellText.length > 15 &&
                (cellText.includes("Qty") || cellText.includes("..."))
              ) {
                longestText = cellText;
              }
            });

          if (longestText && items.length < 5) {
            // Clean up the item name
            let cleanItem = longestText.replace(/\s+Qty\s*:\s*\d+/g, "").trim();
