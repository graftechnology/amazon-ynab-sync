...
const scanEmail = (email) => {
  ...
  try {
    ...
    // New parsing method for updated Amazon email format
    if (items.length === 0) {
      // Look for links to Amazon products (these contain item names)
      $("a").each((i, element) => {
        const href = $(element).attr("href");
        const text = $(element).text().trim();

        // Check if this is a product link and has meaningful text
        if (
          href &&
          ...
          // Skip if it's a generic link text
          if (
            !text.toLowerCase().includes("view") &&
            !text.toLowerCase().includes("edit") &&
            !text.toLowerCase().includes("order") &&
            !text.toLowerCase().includes("account") &&
            !text.toLowerCase().includes("buy again")
          ) {
            // Clean up the item name
            let cleanItem = text.replace(/\.\.\.$/, "...").trim();
            if (cleanItem.length > 10 && !items.includes(cleanItem)) {
    ...
    if (items.length === 0) {
      // Look for the actual order items section (before recommendations)
      let foundOrderSection = false;
      let inRecommendationsSection = false;

      $("table tr, div, td").each((i, element) => {
        const text = $(element).text().trim();

        // Stop parsing if we hit the recommendations section
        if (
          text.toLowerCase().includes("customers who bought") ||
          text.toLowerCase().includes("also bought") ||
          text.toLowerCase().includes("recommended") ||
          text.toLowerCase().includes("you might also like") ||
          text.toLowerCase().includes("more items to consider")
        ) {
          inRecommendationsSection = true;
          return false; // Stop processing
        }

        // Skip if we're in recommendations
        if (inRecommendationsSection) {
          return;
        }

        // Look for actual order items (must have Qty/Quantity and be in order section)
        if (
          (text.includes("Qty") || text.includes("Quantity")) &&
          text.includes(":") &&
          text.length > 15
        ) {
          // Make sure this looks like an actual order item, not a recommendation
          if (
            !text.includes("$") && // Order items don't show price in the item line
            !text.toLowerCase().includes("total") &&
            !text.toLowerCase().includes("shipping") &&
            !text.toLowerCase().includes("tax")
          ) {
            // Clean up the item name (remove Qty/Quantity info)
            let cleanItem = text
              .replace(/\s+(Qty|Quantity)\s*:\s*\d+/g, "")
              .trim();
            if (cleanItem.length > 10 && !items.includes(cleanItem)) {
              items.push(cleanItem);
            }
    ...
    // Fallback: Look for text patterns that look like product names
    if (items.length === 0) {
      $("div, span, td").each((i, element) => {
        const text = $(element).text().trim();

        // Look for long text that might be product names (before hitting recommendations)
        if (
          text.length > 20 &&
          text.length < 200 &&
          !text.toLowerCase().includes("more items to consider") &&
          ...
          !text.toLowerCase().includes("order #") &&
          !text.toLowerCase().includes("arriving") &&
          !text.includes("$") &&
          text.match(/[A-Z]/) && // Has uppercase letters
          !items.includes(text)
        ) {
          // Check if this element is near a "Quantity:" element
          const parent = $(element).parent();
          const siblings = parent.find("*");
          let hasQuantity = false;

          siblings.each((j, sibling) => {
            if ($(sibling).text().includes("Quantity:")) {
              hasQuantity = true;
              return false;
            }
          });
