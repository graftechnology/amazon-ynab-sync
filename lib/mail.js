  ...
  try {
    ...
    if (items.length === 0) {
      ...
      $("table tr, div, td").each((i, element) => {
        ...
        // Look for actual order items (must have Qty and be in order section)
        if (text.includes("Qty") && text.includes(":") && text.length > 15) {
          ...
          if (
            ...
            // Clean up the item name (remove Qty info)
            let cleanItem = text.replace(/\s+Qty\s*:\s*\d+/g, "").trim();
